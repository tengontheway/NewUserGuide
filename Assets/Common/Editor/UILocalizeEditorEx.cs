...
[CustomEditor(typeof(UILocalizeEx), true)]
public class UILocalizeEditorEx : Editor
{
	List<string> mKeys;

	void OnEnable ()
	{
		Dictionary<string, int> dict = Lang.Instance.macroIdList;
	...
	public override void OnInspectorGUI ()
	{
		...
		if (isPresent)
		{
			if (NGUIEditorTools.DrawHeader("Preview"))
			{
				...
				string[] keys = Lang.Instance.SupportLangs;
				string[] values;

				if (Lang.Instance.TryGetValue(myKey, out values))
				{
					...
					else
					{
						for (int i = 0; i < keys.Length; ++i)
						{
							...
							{
								(target as UILocalizeEx).value = values[i];
